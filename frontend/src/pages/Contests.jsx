...
function JoinModal({ contest, userId, username, institution, onClose, onJoined }) {
  ...
  const handleJoin = async () => {
    ...
    try {
      const res = await API.post(`/contests/${contest._id}/join`, {
        access_key: key
      });
      onJoined();
      onClose();
      ...
      else setErr(err.response?.data?.error || result.error || 'Failed to join');
    } finally {
      setLoading(false);
    }
  };
...
function ContestCard({ contest, userId, username, institution, onJoined }) {
  ...
  const [registered, setRegistered] = useState(contest.registered || false);
  const status = computeStatus(contest);
  const isCreator = contest.created_by === userId;

  const handleAction = () => {
    if (status === 'completed') return;
    if (registered && status === 'live') { navigate(`/arena/${contest._id}`); return; }
    setModal(true);
  };
  ...
  const afterJoined = () => {
    ...
    onJoined();
    if (status === 'live') {
      setTimeout(() => navigate(`/arena/${contest._id}`), 500);
    }
  };
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            {registered && <span style={{ fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 20, background: '#dcfce7', color: '#15803d' }}>✓ Registered</span>}
            {isCreator && <span style={{ fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 20, background: '#FEF1EB', color: '#D4521A' }}>👤 You Created</span>}
          </div>
        ...
        <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 13, color: '#888' }}>👥 {(contest.participant_count || (contest.registered_users || []).length)}/{contest.max_participants}</span>
        ...
        {/* CTA */}
          {isCreator ? (
          <button
            onClick={() => navigate(`/contest-dashboard/${contest._id}`)}
            style={{
          ...
          <button
            ...
            style={{
              ...
              background: status === 'completed' ? '#F3F0EA' : status === 'live' ? '#16a34a' : '#888',
              color: status === 'completed' ? '#AAA' : '#fff',
            }}
          >
            {status === 'completed' ? 'Contest Ended' : status === 'live' ? '⚡ Enter Coding Arena →' : '✓ Waiting for Start'}
          </button>
        ) : (
          <button
            onClick={handleAction}
            ...
            style={{
              padding: '11px', borderRadius: 10, border: 'none', fontSize: 14, fontWeight: 800,
              fontFamily: 'inherit', cursor: status === 'completed' || registering ? 'not-allowed' : 'pointer',
              background: status === 'completed' ? '#F3F0EA' : '#D4521A',
              color: status === 'completed' ? '#AAA' : '#fff',
              opacity: registering ? 0.7 : 1,
              transition: 'all 0.3s',
            }}
          >
            {registering ? '⏳ Registering...' : status === 'live' ? '⚡ Join Now' : 'Register Free'}
          </button>
        )}
      </div>
...
function CreateWizard({ userId, username, onCreated, onClose }) {
  ...
  const submit = async () => {
    if (!validate()) return;
    try {
      const res = await API.post('/contests/create', { ...form, problems });
      const id = res.data.contest_id;
      // Also save to local as backup
      createContest({ ...form, problems }, userId, username);
      onCreated(id, res.data.access_key);
    } catch (err) {
      // Fallback to local only
      const id = createContest({ ...form, problems }, userId, username);
      onCreated(id, form.access_key);
    }
  };
...
export default function Contests() {
  ...
  const loadCreatedContests = () => {
    API.get('/contests/my/created').then(r => {
      if (r.data?.contests?.length) setCreatedContests(r.data.contests);
      else setCreatedContests([]);
    }).catch(() => setCreatedContests([]));
  };
  ...
  useEffect(() => { loadContests(); loadCreatedContests(); }, [tick]);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAF8F4', fontFamily: '"DM Sans","Segoe UI",sans-serif' }}>
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 32px 60px' }}>
        ...
        {/* SECTION 1: YOU CREATED */}
        {createdContests.length > 0 && (
          <div style={{ marginBottom: 48 }}>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: '#1A1A1A', margin: '0 0 20px', display: 'flex', alignItems: 'center', gap: 8 }}>
              👤 You Created <span style={{ fontSize: 14, fontWeight: 600, color: '#888' }}>({createdContests.length})</span>
            </h2>
        ...
        {/* SECTION 2: ALL CONTESTS */}
        <div>
          <h2 style={{ fontSize: 18, fontWeight: 800, color: '#1A1A1A', margin: '0 0 20px', display: 'flex', alignItems: 'center', gap: 8 }}>
            🏆 Contests <span style={{ fontSize: 14, fontWeight: 600, color: '#888' }}>({contests.length})</span>
          </h2>
      ...
      {showCreate && (
        <CreateWizard
          ...
          onCreated={(id, key) => {
            ...
            loadContests();
            loadCreatedContests();
            if (key) alert(`Contest created! Access key: ${key}\nShare this with participants.`);
